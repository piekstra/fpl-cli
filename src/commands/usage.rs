...
pub fn run(ctx: &Ctx, cmd: &UsageCommand) -> Result<(), AppError> {
    ...
    match cmd {
        UsageCommand::Get { account_id } => {
            ...
            output::emit(ctx.cli.json, &fpl.energy_usage(&account, &c.premise, &c.last_billed, &c.meter)?, output::usage_summary);
        }
        UsageCommand::Hourly { account_id, date } => {
            ...
            output::emit(ctx.cli.json, &fpl.hourly_usage(&account, &c.premise, &day)?, output::hourly);
        }
        UsageCommand::Appliances { account_id } => {
            let account = ctx.resolve_account(account_id.as_deref(), &fpl)?;
            let c = account_ctx(&fpl, &account)?;
            output::emit(ctx.cli.json, &fpl.appliance_usage(&account, &c.premise)?, output::appliances);
        }
    }
    Ok(())
}
